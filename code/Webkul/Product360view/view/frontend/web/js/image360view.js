        ...
        $.widget(
            'webkul.image360view',
            {
                _create: function () {
                    ...
                    var self = this;
                    var configFrameTime = self.options.imageTag.frameTime;
                    var mediaImagecount = self.options.imageTag.mediaImagecount;
                    var product360images = JSON.parse(self.options.imageTag.product360images.replace(/&quot;/g,'"'));
                    ...
                    $('body').attrchange({
                        ...
                        callback: function(e) {
                            ...
                            if (oldValue.includes("fotorama__fullscreen")) {
                                $("#maincontent").find('.wk_product360--thumb').remove();
                                addThumbClass();
                            }
                            if (newValue.includes("fotorama__fullscreen")) {
                                $("body").find('.wk_product360--thumb').remove();
                            }
                        }
                      });
                    ...
                    function addThumbClass()
                    {
                        if (mediaImagecount > 1) {
                            $(self.options.imageTag.imagePath).insertBefore(".fotorama__nav__shaft");
                            $("#maincontent").find('.fotorama__nav__shaft').addClass("wk-product360");
                        } else {
                            $(self.options.imageTag.imagePath).insertBefore(".fotorama__nav__shaft");
                            $("#maincontent").find('.fotorama__nav__shaft').addClass("wk-product360");
                            $(".fotorama__nav-wrap").css('display','block');
                            $(".fotorama__nav").css('display','block');
                            $(".fotorama__thumb__arr, .fotorama__thumb-border").css('display','none');
                        }
                    }

                        $('body').on('click', '.wk_product360--thumb', function(event) {
                            $('.wk_startpause_btn').addClass("paused");
                            $('.content-360view').modal({
                                'type': 'slide',
                                buttons: [],
                            });
                            $('.content-360view').modal('openModal');
                            $(function(){
                                $("#view360").spritespin({
                                    // path to the source images.
                                    source: product360images,
                                    animate: true,
                                    frameTime: configFrameTime,
                                    responsive: true,
                                    sense: -1,
                                    plugins: [
                                        '360',
                        ...
                        $(".spritespin-instance").on("click", function(){
                            autorotate = false;
                        });

                        $(".wk_startpause_btn").on("click", function(){
                            if (autorotate) {
                                $('.wk_startpause_btn').removeClass("paused");
                                autorotate = false;
                            } else {
                                $('.wk_startpause_btn').addClass("paused");
                                autorotate = true;
