...
class w3speedup_optimize_image {
	public function __construct(){
		parent::__construct();
	}
	
	function w3speedup_optimize_single_image($metadata, $attachment_id, $context){
	...
	function w3_increment_prioritized_img($attach_id){
		$opt_priority = get_option('w3speedup_opt_priortize');
		...
		update_option('w3speedup_opt_priortize',$opt_priority,'no');
		return true;
	}
	...
	function w3speedup_optimize_image_callback(){
		
		$total_count = (int)$this->get_option("w3_images_count",0);
		$offset_limit = !empty($_REQUEST['w3_limit']) && (int)$_REQUEST['w3_limit'] > 0 ? (int)$_REQUEST['w3_limit'] : 2;
		$attach_arr = $this->get_option("w3_images",1);
		$images= array();
		if(!empty($attach_arr) && count($attach_arr) > 0){
			$i = 0;
			foreach($attach_arr as $key=>$attach_id){
				$this->w3_optimize_attachment($attach_id);
				$images[] = $attach_id;
			...
			$this->update_option('w3_images',$attach_arr,1);
		}
		echo json_encode(array_merge(array('offset'=>$total_count-count($attach_arr)),$images));
		exit;
	}
	function w3_optimize_attachment($image_url_path,$overwrite=false){
		...
		$image_url = str_replace($this->add_settings['upload_base_dir'],$this->add_settings['upload_base_url'],$image_url_path);
		...
		if( $optimize_image && in_array($type[0],$image_type) && !is_file($image_url_path.'org.'.$type[0]) ){
			if($image_size[0] > 1920){
				...
				$image_size[0] = 1920;
				$this->w3speedup_resize_image( $this->add_settings['document_root'].$url_array['path'].'org.'.$type[0], $this->add_settings['document_root'].$url_array['path'], $image_size[0]);*/
				return $return;
			}
	...
	function w3speedup_resize_image( $file, $dest_path, $max_w) {
