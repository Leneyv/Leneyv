...
class Collection extends \Magento\Quote\Model\ResourceModel\Quote\Collection
{
    ...
    public function resolveCustomerNames()
    {
        ...
        $customerName = $this->customerResource->getConnection()->getConcatSql(['customer.firstname', 'customer.lastname'], ' ');

        /* $select->reset()
            ->from(['customer' => $this->getTable('customer_entity')], '')
            ->columns('customer.entity_id,customer.email,')
            ->columns('cusattr.telephone')
            ->joinInner(
                ['cusattr' => $this->getTable('customer_address_entity')],
                'customer.entity_id = quote.parent_id'
            )->where(
...
*/
        ...
        $select->from(
            ['customer' => $this->customerResource->getTable('customer_entity')],
            ['customer.entity_id', 'customer.email' ]
        );
        ...
        $select-> joinLeft
        (   ['cusattr' => $this->getTable('customer_address_entity')],
                'customer.entity_id = cusattr.parent_id'
            );
        $select->where(
            'customer.entity_id IN (?)',
            array_column(
       ...
       $customersData = $this->customerResource->getConnection()->fetchAll($select);
        ...
        foreach ($this->getItems() as $item) {
            foreach ($customersData as $customerItemData) {
              ...
              /* echo $item['customer_id'] .'-'.$customerItemData['entity_id'];
                echo '</br>'; */
                
                if($customerItemData['parent_id'] != "")
                {
                    if ($item['customer_id'] == $customerItemData['parent_id']) {
                       
                        $item->setData(array_merge($item->getData(), $customerItemData));
                    }
                }
                else if($customerItemData['parent_id'] != "")
                {
