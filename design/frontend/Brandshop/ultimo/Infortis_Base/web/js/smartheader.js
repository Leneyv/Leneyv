;(function ($, window, document, undefined) {
    ...
    $.widget("infortis.smartheader", {
        ...
        , _moveElementsToMobilePosition: function()
        {
            ...
            var target = $('.nav-mobile.nav-acco.nav-mobile-triggerable .nav-panel-inner .header-nav-dropdown-wrapper h4 a');

            $(target).click(function(event)
            {
                if($(this).parent().next('h6').length){
                    return false;
                }
            });
            $('<span class="close"></span>').prependTo('li.level-top .nav-panel-inner');
            
            $(document).on('click','.nav-panel.nav-panel--dropdown.full-width.is-active .nav-panel-inner span.close',function(event)
            {
                $('.nav-panel.nav-panel--dropdown.full-width').removeClass('is-active');
            });
